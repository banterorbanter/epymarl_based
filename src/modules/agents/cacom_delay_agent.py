...
class CACOM_Agent(nn.Module):
    def __init__(self, input_shape, args):
        ...
        self.gate_buffer = [dict() for _ in range(args.n_agents)]
        ...
        **kwargs
        ...
        if test_mode:
            ...
            for i in range(self.n_agents):
                self.msg_buffer[i][t + latency[0][i].item()] = response[:,:,i,:]

            d_response = th.zeros_like(response)
            for i in range(self.n_agents):
                if t in self.msg_buffer[i]:
                    d_response[:,:,i,:] = self.msg_buffer[i].pop(t)
            response = d_response
    ...
    def cal_gate_labels(self, inputs, hidden_state, bs, exp_gate, t):
        with th.no_grad():
            feat, requests = self.pre_comm(inputs, hidden_state, bs)
            # (bs, reply_agents, request_agents, response_dim)
        ...
        with th.no_grad():
            ...
            # for i in range(self.n_agents):
            #     self.gate_buffer[i][t + latency[0][i].item()] = response[:, :, i, :]

            # d_response = th.zeros_like(response)
            # for i in range(self.n_agents):
            #     if t in self.gate_buffer[i]:
            #         d_response[:, :, i, :] = self.gate_buffer[i].pop(t)
