...
class MASIAAgent(nn.Module):
    ...
    def __init__(self, input_shape, args):
        ...
        self.z_buffer = [dict() for _ in range(args.n_agents)]
    ...
    def forward(self, inputs, hidden_state, encoder_hidden_state, t, test_mode=False):
        ...
        if self.args.noise_env and self.args.noise_type == 1:
            ...
            repeated_z = z.unsqueeze(1).repeat(1, self.args.n_agents, 1).reshape(bs*self.args.n_agents, -1) # [bs*n_agents, state_repre_dim*n_agents]
        ...
        if test_mode:
            ...
            t_weight_z = weighted_z.view(bs, self.args.n_agents, self.args.n_agents, -1)
            for i in range(self.args.n_agents):
                self.z_buffer[i][t + latency[0][i].item()] = t_weight_z[:,:,i,:]

            d_weighted_z = th.zeros_like(t_weight_z)
            for i in range(self.args.n_agents):
                if t in self.z_buffer[i]:
                    d_weighted_z[:,:,i,:] = self.z_buffer[i].pop(t)
                d_weighted_z[:,i,i,:] = t_weight_z[:,i,i,:]
            weighted_z = d_weighted_z.view(bs * self.args.n_agents, -1)
